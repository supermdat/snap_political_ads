...
data_20182019_long <-
  data_20182019 %>% 
  mutate(payingadvertisername = payingadvertisername %>% 
           str_squish() %>% 
           str_trim(side = "both") %>% 
           tolower(),
         interests_long = str_split(interests,
                                    ",",
                                    ),
...
# glimpse(us_only_vars_abv_pt5_missing)
...
```{r}
sum_stats_advertiser %>% filter(spend_med_rownum_entire_group <= 10) %>% View()
```
...
func_plot <-
  function(data, 
    ...
    plot =
      (data %>% 
         filter({{var_color}} %in% top_n_filter) %>% 
         ggplot(aes(x = {{var_x}},
                    y = {{var_y}},
                    color = {{var_color}}
                    )
