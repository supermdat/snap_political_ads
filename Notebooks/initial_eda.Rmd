...
# other
library("here")           # simpler file paths to not explicitly set the working directory
...
First, get the unique zip codes to lookup.
```{r}
...
#          ) %>% 
#   select(postal_codes__included__long) %>% 
#   mutate_all(as.character) %>% 
#   distinct() %>% 
#   pull(postal_codes__included__long)
# 
# unique_zips_exl <-
#   join_unlisted_data %>% 
#   filter(!is.na(postal_codes__excluded__long)
#          ) %>% 
#   select(postal_codes__excluded__long) %>% 
#   mutate_all(as.character) %>% 
#   distinct() %>% 
#   pull(postal_codes__excluded__long)
# 
# zips_all <- c(unique_zips_inc, unique_zips_exl)
# 
# unique_zips <- unique(zips_all)
# 
# str(unique_zips)
# 
# 
# rm(unique_zips_inc, unique_zips_exl)

```


Now, get the data from the HUD API.
```{r}

# url_base <- "https://www.huduser.gov/hudapi/public/usps"
# 
# 
# get_call <-
#   unique_zips %>% 
#   map(~ GET(url = paste0(url_base,
#                          "?type=1&query=",
#                          .x
#                          ),
#             add_headers(Authorization = paste0("Bearer ",
#                                                Sys.getenv("HUD_USPS_API_KEY")
#                                                )
#                         )
#             )
#       )
...
# api_content <-
#   get_call %>% 
#   map(~ jsonlite::fromJSON(txt = content(x = .x,
#                                          as = "text"
#                                          ),
#                            simplifyVector = TRUE,
...
# 
# api_data <-
#   pmap(.l = list(a = api_content),
#        .f = function(a) {
#          z = a$data$input
#          
#          result = a$data$results %>% 
#            mutate(zip = z)
#          
#          return(result)
#          }
#        )
# 
# test_content$data$results %>% View()
# 
# test_content$data$results %>% 
#   mutate(zip = test_content$data$input)
...
# View(zip_to_tract_raw %>% filter(ZIP == "20016"))
