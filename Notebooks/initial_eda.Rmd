...
func_data_line_plot <-
  function(data, 
    ...
    top_advertisers =
      data_cnts %>%
      filter(cnt >= cnt_cutoff) %>%
      pull({{grp_var}})
    
    data_cnts =
      data_cnts %>% 
      filter({{grp_var}} %in% top_advertisers)

    plot =
      if(missing(time_var)
         ) {
        (data_cnts %>% 
           ggplot(aes(x = fct_reorder({{grp_var}},
                                      cnt
                                      ),
                      y = cnt
                      )
           ...
           coord_flip() +
           theme_minimal() +
           labs(x = paste0(quo_get_expr(quo({{grp_var}})
                                        )
                           )
                ) +
           NULL
...
test <-
  us_only_vars_abv_pt5_missing %>% 
  func_data_line_plot(
                      # grp_var = payingadvertisername,
                      grp_var = state_nameincl,
                      # grp_var = postal_codes__included__long,
                      # zip_cnt = TRUE,
